...
const onGotoFullPage = () => {
	...
	if (verifyUrl(<string>state.permission)) window.open(state.permission);
	else window.open(`${origin}${pathname}#${state.permission}`);
};
// 监听路由的变化，设置内容
watch(
	() => route.path,
	() => {
		...
		state.permission = <string>route.meta.permission;
	},
	{
		immediate: true,
	}
