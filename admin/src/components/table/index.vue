<template>
	<div class="table-container">
		...
		>
			...
			<el-table-column label="操作" width="150" v-if="config.isOperate">
				<template v-slot="scope">
					<el-button text type="warning" size="small"  v-if="config.isThree" @click="onThreeRow(scope.row)">{{config.isThree}}</el-button>
					<el-button text type="primary" size="small"  v-if="auth(config.isUpdate)" @click="onUpdateRow(scope.row)">修改</el-button>
					<el-popconfirm title="确定删除吗？" @confirm="onDelRow(scope.row)">
						<template #reference>
							<el-button text size="small" type="danger" v-if="auth(config.isDelete)">删除</el-button>
						</template>
					</el-popconfirm>
				</template>
			</el-table-column>
