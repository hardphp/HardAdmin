...
<script setup lang="ts" name="toolApp">
import { defineAsyncComponent, reactive, ref, onMounted } from 'vue';
import { ElMessage } from 'element-plus';
import { getList,del,change } from '/@/api/app/index';

// 引入组件
const Table = defineAsyncComponent(() => import('/@/components/table/index.vue'));
const TableSearch = defineAsyncComponent(() => import('/@/components/table/search.vue'));
const EditDialog = defineAsyncComponent(() => import('/@/views/tool/app/form/edit.vue'));
...
const state = reactive<TableDemoState>({
	tableData: {
		...
		header: [
			...
			{ key: 'appid', colWidth: '', title: 'appid', type: 'text',  isCheck: true },
			{ key: 'app_salt', colWidth: '300', title: '密钥', type: 'text', isCheck: true },
			{ key: 'is_enabled', colWidth: '150', title: '状态', type: 'radio',dict:{'1':'正常','0':'禁用'},isCheck: true },
			{ key: 'is_sign', colWidth: '150', title: '验签', type: 'radio', dict:{'1':'是','0':'否'},isCheck: true },
		...
		config: {
			...
			isAdd: 'admin:app:save', //新增标识
			isUpdate: 'admin:app:save', //修改标识
			isDelete: 'admin:app:del', //删除标识
